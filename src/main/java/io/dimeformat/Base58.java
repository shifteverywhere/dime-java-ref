...
public class Base58 {
    ...
    public static String encode(byte[] data) {
        if (data == null || data.length == 0) { throw new IllegalArgumentException("Key to encode must not be null or of zero length."); }
        try {
            byte[] bytes = Utility.combine(data, generateChecksum(data));
            String b1 = Utility.toHex(bytes);
            BigInteger bigInt = BigInteger.valueOf(0);
            for (int i = 0; i < bytes.length; i++) {
                bigInt = bigInt.multiply(BigInteger.valueOf(256)).add(BigInteger.valueOf(bytes[i]));
            }

            StringBuffer buffer = new StringBuffer();

            while (bigInt.compareTo(BigInteger.valueOf(0)) == 1) {
                BigInteger[] result = bigInt.divideAndRemainder(BigInteger.valueOf(58));
                bigInt = result[0];
                buffer.insert(0, Base58._indexTable[result[1].intValue()]);
            }
    ...
    private static final char[] _indexTable = { '1', '2', '3', '4', '5', '6', '7', '8', '9', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z' };

    private static byte[] generateChecksum(byte[] data) throws UnexpectedException {
        try {
            byte[] hash1 = Base58.sha256(data);
            byte[] hash2 = Base58.sha256(hash1);
            byte[] result = Utility.subArray(hash2, 0, Base58.CHECKSUM_SIZE);
            return result;
        } catch (Exception e) {
            throw new UnexpectedException("Unexpected exception thrown (B1001).", e);
        }
    }
